...
def main():
    parser = argparse.ArgumentParser(description="View tracked Gamma presentations")

    parser.add_argument("--stats", action="store_true", help="Show statistics only")

    parser.add_argument(
        "--search", type=str, help="Search for presentations by title or file"
    )
    ...
    if args.search:
        ...
        presentations = [
            ...
            if (
                search_lower in p.get("title", "").lower()
                or search_lower in p.get("local_file", "").lower()
            )
        ]
